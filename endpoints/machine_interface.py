...
class MachineInterface(object):
    def __init__(self, root, w, h, is_automode=False, model_file=os.path.join('models', 'default.pth'),
        ...
        if not self.is_automode:
            ...
            self.state_info = tk.Label(self.canvas, text=self.humanoid_form, font=("Arial", 20))
            self.state_info.pack(side=tk.TOP)
    ...
    def suggest(self, humanoid, scorekeeper, capacity_full = False):
        scorekeeper.update_time(7)
        if self.is_model_loaded:
            state, action = self.get_model_suggestion(humanoid, capacity_full)
        else:
    ...
    def act(self, scorekeeper, humanoid, capacity_full=False):
        scorekeeper.update_time(7)
        state, action = self.get_model_suggestion(humanoid,
                                                  capacity_full)  # fixed bug if you don't suggest before: wrong action
    ...
    def humanoid_info(self, scorekeeper, humanoid, capacity_full=False):
        scorekeeper.update_time(5)
        predicted_state, action = self.get_model_suggestion(humanoid, capacity_full)
        self.humanoid_form = predicted_state.name
        self.state_info.config(text=self.humanoid_form)
    ...
    def get_model_suggestion(self, humanoid, is_capacity_full) -> ActionCost:
        ...
        return predicted_state, recommended_action
