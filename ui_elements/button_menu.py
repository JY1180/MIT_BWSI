...
class ButtonMenu(object):
    ...
    def disable_buttons(self, remaining_time, remaining_humanoids, at_capacity, too_many_zombies):
        if remaining_humanoids == 0 or remaining_time <= 0 or too_many_zombies:
            ...
            if at_capacity or remaining_time <= 0:
                self.buttons[0].config(state="disabled")
                self.buttons[1].config(state="disabled")
                self.buttons[2].config(state="disabled")
                self.buttons[4].config(state="disabled")
                self.buttons[5].config(state="disabled")
                # I added the else statement because if you delete it, once you scram once, you cannot do it again
            else:
