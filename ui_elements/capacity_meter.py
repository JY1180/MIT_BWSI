...
class CapacityMeter(object):
    ...
    def render(self, max_cap, size):
        ...
        for i in range(0, max_cap):
            self.units.append(create_unit(self.canvas, x, y, size))
            x += size * 1.5
    ...
    def update_fill(self, index):
        if index != 0:
            self.canvas.itemconfig(self.units[index - 1], fill="", stipple="")
        else:
            for unit in self.units:
                self.canvas.itemconfig(unit, fill='midnightblue', stipple="gray25")
