...
class Main(object):
    ...
    def __init__(self, is_automode, is_disable):
        ...
        if not is_automode:  # Launch UI gameplay
            ...
            while len(self.data_parser.unvisited) > 0:  # number of images
                if self.scorekeeper.remaining_time <= 0:
                    break
                else:
                    humanoid = self.data_parser.get_random()
                    simon.suggest(humanoid, self.scorekeeper, capacity_full=self.scorekeeper.at_capacity())
                    simon.act(self.scorekeeper, humanoid)
                    simon.humanoid_info(self.scorekeeper, humanoid)
